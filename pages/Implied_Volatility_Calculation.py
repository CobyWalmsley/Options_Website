...
if not os.path.exists(img_path):
    ...
    <div class="lehigh-sticky-banner">
        ...
        <div>
            <div class="title">Options and Volatility | Implied Volatility Calculation</div>
...
with col2:
    with st.form("calc_form"):
        submitted = st.form_submit_button("Calculate Implied Volatility")
        
        option_price = st.number_input(f"Contract Price",min_value=0)
        ...
        if submitted:
            try:
                with st.spinner("Calculating..."):
                    if contract_type=='Call':
                        implied_vol=e_call_vol(stock_price,strike,days_to_maturity/365,risk_free_rate,option_price)
                    if contract_type=='Put':
                        implied_vol=e_put_vol(stock_price,strike,days_to_maturity/365,risk_free_rate,option_price)
    ...
    with col1:
        if "implied_vol" in st.session_state:
            implied_vol = st.session_state.get("implied_vol")
           
            st.title(f"Implied Volatility: {implied_vol}")

st.title('Implied Volatility')
st.write('Why is it called implied volatility in the first place?')
st.write("The reason is that the volatility variable present in option pricing models is always future volatility, which can never be known for certain.")
st.write('The volatility is "implied" by the option contract price. Market makers forecast a future volatility and use it to price their options contracts.')
